...
function DragonCursorImpl() {
  ...
  useEffect(() => {
    // Update mobile status on layout/mount
    const checkMobile = () => {
    ...
    let lastTimePointerMoved = Date.now();
    ...
    const run = () => {
      ...
      const dx = ax + pointer.x - e0.x;
      const dy = ay + pointer.y - e0.y;
      ...
      for (let i = 1; i < N; i++) {
        let eCur = elems[i];
        let ep = elems[i - 1];
        const a = Math.atan2(eCur.y - ep.y, eCur.x - ep.x);
        const spacing = ((100 - i) / 5) * scaleMultiplier;
        
        const targetX = ep.x + Math.cos(a) * spacing;
        const targetY = ep.y + Math.sin(a) * spacing;
        
        const diffX = targetX - eCur.x;
        const diffY = targetY - eCur.y;
        
        eCur.x += diffX / 4;
        eCur.y += diffY / 4;
        
        maxDiff = Math.max(maxDiff, Math.abs(diffX), Math.abs(diffY));
        
        const scaleFactor = 3.24 - (3.04 * (i - 1)) / (N - 2);
        const s = scaleFactor * scaleMultiplier;
 
        if (eCur.use) {
          eCur.use.setAttributeNS(
            null,
            "transform",
            `translate(${(ep.x + eCur.x) / 2},${(ep.y + eCur.y) / 2}) rotate(${
              (180 / Math.PI) * a
            }) scale(${s},${s})`
          );
        }
      ...
      // If the mouse hasn't moved in a while AND all elements have settled down to sub-pixel coordinates
      if (Date.now() - lastTimePointerMoved > 1500 && maxDiff < 0.25) {
        isLoopRunning = false;
        return;
      }
  ...
  return (
    <svg
      ref={containerRef}
      className={`fixed inset-0 z-[9999] w-full h-full pointer-events-none transition-opacity duration-700 transform-gpu will-change-transform ${
        isMobileSize ? '' : 'filter drop-shadow-[0_4px_16px_rgba(240,90,40,0.45)]'
    ...
    >
      <defs>
        <g id="Cabeza" transform="matrix(1, 0, 0, 1, 0, 0)">
          ...
          />
          <path
            style={{ fill: '#f05a28', fillOpacity: 1 }}
        ...
        <g id="Aletas" transform="matrix(1, 0, 0, 1, 0, 0)">
          <linearGradient
            ...
            x2="819.2"
            y2="0">
            <stop offset="0" style={{ stopColor: '#f05a28', stopOpacity: 1 }} />
            <stop offset="1" style={{ stopColor: '#111111', stopOpacity: 1 }} />
          </linearGradient>
          <path
        ...
        <g id="Espina" transform="matrix(1, 0, 0, 1, 0, 0)">
          <linearGradient
            ...
            y2="0">
            <stop offset="0" style={{ stopColor: '#f05a28', stopOpacity: 1 }} />
            <stop offset="1" style={{ stopColor: '#222222', stopOpacity: 1 }} />
          </linearGradient>
          <path
