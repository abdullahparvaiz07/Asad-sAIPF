...
export function DragonCursor() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      for (let i = 1; i < N; i++) {
        ...
        eCur.x += (ep.x - eCur.x + (Math.cos(a) * (100 - i)) / 5) / 4;
        eCur.y += (ep.y - eCur.y + (Math.sin(a) * (100 - i)) / 5) / 4;
  ...
  if (isMobile || !isDragonActive) return null;
  ...
  return (
    <svg
      ref={containerRef}
      className={`fixed inset-0 z-[9999] w-full h-full pointer-events-none transition-opacity duration-700 filter drop-shadow-[0_4px_16px_rgba(240,90,40,0.45)] transform-gpu will-change-transform ${
